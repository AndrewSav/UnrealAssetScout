...
- `-d`, `--no-audio-disambiguation`: `export audio:` Audio exports suffix each Wwise media file with the id of the container it was read from, so that media CUE4Parse names alike do not overwrite each other. CUE4Parse names extracted media after the source asset plus the Wwise language, and every non-localised sound shares the language `SFX`, so distinct media end up with the same name. This option drops the suffix and reverts to stock FModel/CUE4Parse naming, at the cost of keeping only the last-written of any media that collide; the run reports every such file and the containers it came from, so nothing is lost silently. This is ignored for other export modes.
