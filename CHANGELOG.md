...
- Updated CUE4Parse to the latest commit. It fixes an intermittent access violation (0xC0000005) during exports that load animations. The native buffer holding compressed animation data could be freed by the garbage collector while it was still being decompressed, so the crash showed up only under sustained memory pressure and never on a single asset (FabianFG/CUE4Parse#441). Output may be different on the same input.
